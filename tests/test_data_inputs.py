...
def test_volume_init_data():
    vol_card = parse_data("VOL 1 1 2J 0")
    answers = [1.0, 1.0, Jump, Jump, 0.0]
    for i, vol in enumerate(vol_card._volume):
        if isinstance(answers[i], float):
            assert vol == pytest.approx(answers[i])
        else:
            assert isinstance(vol, Jump)
    # test starting jump
    vol_card = parse_data("VOL 2J 1 0")
    ...
    for i, vol in enumerate(vol_card._volume):
        if isinstance(answers[i], float):
            ...
            assert isinstance(vol, Jump)
    # tests starting no
    vol_card = parse_data("VOL NO 1 1 2J 0")
    assert not vol_card.is_mcnp_calculated
    # invalid number
    with self.assertRaises(MalformedInputError):
        vol_card = parse_data("VOL NO s 1 2J 0")
    # negative volume
    with self.assertRaises(MalformedInputError):
        vol_card = parse_data("VOL NO -1 1 2J 0")
...
def test_volume_init_data():
    ...
    for i, vol in enumerate(vol_card._volume):
        if isinstance(vol, syntax_node.ValueNode):
            assert vol.value == pytest.approx(answers[i])
        else:
            assert isinstance(vol, Jump)
